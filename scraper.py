                            ...
                            for items in EscortDataList :
                                if len (re.findall('Medidas', items)) :
                                    if len (re.findall(r'\<td\sclass\=\"RespPop\"\>', items)) :
                                        ...
                                        medidas = medidas.replace('\t','')
                                        print 'Medidas :\n------------------------------------\n', medidas, '\n------------------------------------\n'
                                        if (len (re.findall(r'\d+\-\d+\-\d+', medidas))) && ( ~(len (re.findall(r'\&', medidas)))) && ( ~(len (re.findall(r'\#', medidas)))):
                                            pechos = re.findall(r'(\d+)\-\d+\-\d+', medidas)[0]
                                            cintura = re.findall(r'\d+\-(\d+)\-\d+', medidas)[0]
                                            caderas = re.findall(r'\d+\-\d+\-(\d+)', medidas)[0]
                                        elif (len (re.findall(r'\d+\W+\d+\W+\d+', medidas))) && ( ~(len (re.findall(r'\&', medidas)))) && ( ~(len (re.findall(r'\#', medidas)))):
                    ...
                    if ( len (re.findall('\/escort\/\d+\/', url))) :
                        ...
                        try:
                            ...
                            if len (re.findall('Consultar', EscortPage)) :
                                ...
                                #print 'tarifaBlock :\n----------------------------------\n', tarifaBlock, '\n--------------------------\n'
                                if len (re.findall('\d+\.\d+', tarifaBlock)) :
                                    tarifa = re.findall('\d+\.\d+', tarifaBlock)[0]
                            tarifa = tarifa.replace('.','')
                            if not len (re.findall(r'\d+', tarifa)) :
                                tarifa = 'null'
                            print 'Tarifa :', tarifa
                    ...
                    if len(re.findall(r'nombre', EscortData)) :
                        ...
                        if len (re.findall(r'movimiento', EscortPage)) :
                            ...
                            if len (re.findall(r'[Pp]art\s[Tt]ime', horario)) :
                                hastaDia = 'Vi'
                                if len (re.findall(r'\d+\W+\d+', horario)) :
                                	inicio = re.findall(r'\d+\W+\d+', horario)[0]
					if len (re.findall(r'\d+\W+\d+', horario))==2 :
						termino = re.findall(r'\d+\W+\d+', horario)[1]
				elif len (re.findall(r'\d+\W+\w+\W+\d+', horario)) :
					inicio = re.findall(r'(\d+)\W+\w+\W+\d+', horario)[0] + ':00'
					termino = re.findall(r'\d+\W+\w+\W+(\d+)', horario)[0] + ':00'
